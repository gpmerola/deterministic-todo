...
Le viste attive osservano uno stream SQLite che esclude completate e tombstone; Completate usa uno stream separato. Anche gli eventuali record marcati `reference` dalla breve versione 2.18.0 entrano nello stream normale, così titolo, note e link restano accessibili e possono essere spostati nei Progetti. Gli stream sono creati una volta per shell e supportati dagli indici della migrazione locale 2. Questo mantiene fuori dalla RAM la cronologia non pertinente.
...
Ogni transazione utente aggiorna SQLite e aggiunge un'operazione all'outbox persistente. `operation_id` è UUID e il server lo registra con vincolo univoco: ritentare dopo crash non duplica l'operazione. Le ricevute sono immutabili e il retry usa `ON CONFLICT DO NOTHING`, coerentemente con la RLS che consente inserimento e lettura ma non alterazione della cronologia. L'outbox avvia subito il push; gli eventi PostgreSQL Realtime filtrati per `user_id` accodano soltanto gli ID modificati. Dopo 120 ms il ricevente esegue query `IN` separate per task, progetti e sezioni e applica il merge Lamport; gli stream Drift aggiornano quindi la UI senza refresh. Chiusure, errori e timeout del canale causano una nuova sottoscrizione dopo due secondi; la sottoscrizione riuscita forza un pull completo. Il timer di dieci minuti è una seconda rete di recupero in foreground per eventi persi, rete assente o processi sospesi; Realtime, outbox, connettività e resume restano immediati. Eventi arrivati durante un sync impostano un secondo passaggio, evitando finestre di perdita. L'ack rimuove l'elemento solo dopo conferma remota.
